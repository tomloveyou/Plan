...
import com.example.hp.testapplication.projection.Point;
...
public class DefaultClusterRenderer<T extends ClusterItem>
    ...
    /**
     * Instantiates a new Default cluster renderer.
     *
    ...
    public DefaultClusterRenderer(Context context, BaiduMap map, ClusterManager<T> clusterManager) {
		...
		mIconGenerator.setBackground(context.getResources().getDrawable(R.drawable.default_ptr_rotate));
    ...
    public void removeMapMarker(Marker marker) {
		...
		mClusterToMarker.remove(getCluster(marker));
		mClusters.remove(getCluster(marker));
		mMarkerCache.remove(marker);
		mMarkerToCluster.remove(marker);
		//mClusterManager.getMarkerManager().removeMarker(marker);
...
//					}
     ...
     *
     * @param clusterItem the cluster item
     * @param marker      the marker
     */
	...
	private class CreateMarkerTask {
        ...
        /**
         * Instantiates a new Create marker task.
         *
